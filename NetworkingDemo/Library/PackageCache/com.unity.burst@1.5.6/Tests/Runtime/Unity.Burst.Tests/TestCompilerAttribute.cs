...
#pragma warning restore 0414
        ...
        protected bool SupportException => false;
        ...
        protected TestCompilerCommandBase GetTestCommand(TestCompilerAttribute attribute, Test test,
        ...
        internal abstract class TestCompilerCommandBase : TestCommand
        {
            ...
            protected TestCompilerCommandBase(TestCompilerAttribute attribute, Test test, TestMethod originalMethod) :
                base(test)
            {
            ...
            protected Type CreateNativeDelegateType(Type returnType, Type[] arguments, out bool isInRegistry,
                out Func<object, object[], object> caller)
            {
...
#else
                    throw new Exception(
                        "Couldn't find delegate in static registry and not able to use a dynamic call.");
#endif
                }
            }

            private static Func<object, object[], object> StaticDynamicDelegateCaller =
                new Func<object, object[], object>((del, arguments) => ((Delegate) del).DynamicInvoke(arguments));
            ...
            private unsafe TestResult ExecuteMethod(ExecutionContext context)
            {
                ...
                if (!RunningArmTestOnIntelCPU(methodInfo))
                {
                    ...
                    TransformArguments(_originalMethod.Method.MethodInfo, arguments, out nativeArgs, out nativeArgTypes,
                        returnBox, out Type returnBoxType);

                    bool isInRegistry = false;
                    Func<object, object[], object> nativeDelegateCaller;
                    var delegateType = CreateNativeDelegateType(_originalMethod.Method.MethodInfo.ReturnType,
                        nativeArgTypes, out isInRegistry, out nativeDelegateCaller);
                    if (!isInRegistry)
                    {
                        TestContext.Out.WriteLine(
                            $"Warning, the delegate for the method `{_originalMethod.Method}` has not been generated");
                    }
                    ...
                    {
                        if (TryExpectedException(context,
                                () => _originalMethod.Method.Invoke(context.TestObject, arguments), ".NET",
                                type => type == _expectedException, _expectedException.FullName, false) !=
                            TryExpectedExceptionResult.ThrewExpectedException)
                        {
                            return context.CurrentResult;
                        }

                        if (TryExpectedException(context, () => nativeDelegateCaller(compiledFunction, nativeArgs),
                                "Native", type => type == _expectedException, _expectedException.FullName, true) !=
                            TryExpectedExceptionResult.ThrewExpectedException)
                        {
                            return context.CurrentResult;
                        }
                    ...
                    else
                    {
                        ...
                        {
                            TestContext.Out.WriteLine(
                                $"Using OverrideManagedResult: `{overrideManagedResult}` to compare to burst `{resultNative}`, managed version not run");
                        ...
                        {
                            TestContext.Out.WriteLine(
                                $"Using OverrideResultOnMono: `{overrideResultOnMono}` instead of `{resultClr}` compare to burst `{resultNative}`");
                        ...
                        {
                            TestContext.Out.WriteLine(
                                $"Using OverrideOn32BitNative: '{overrideOn32BitNative}' instead of '{resultClr}' compare to burst '{resultNative}' due to 32bit native runtime");
                    ...
                    Assert.True(isInRegistry,
                        "The test method is not in the registry, recompile the project with the updated StaticDelegateRegistry.generated.cs");
            ...
            protected void PostAssert(ExecutionContext context)
            {
            }

            protected void ProcessNativeResult(TestMethod method, object result)
            {
            }
            ...
            protected unsafe void TransformArguments(MethodInfo method, object[] args, out object[] nativeArgs,
                out Type[] nativeArgTypes, byte* returnBox, out Type returnBoxType)
            {
                returnBoxType = null;
                ...
                for (var i = 0; i < parameters.Length; i++)
                {
                    ...
                    if (typeof(IFunctionPointerProvider).IsAssignableFrom(expectedArgType) ||
                        (expectedArgType.IsByRef &&
                         typeof(IFunctionPointerProvider).IsAssignableFrom(expectedArgType.GetElementType())) &&
                        actualNativeArgType == typeof(string))
                    {
                        ...
                        if (candidates == null || candidates.Length != 1)
                        {
                            throw new ArgumentException(
                                $"Could not resolve an unambigoues static method from name {methodName}.");
                        }

                        var functionPointer = CompileFunctionPointer(candidates[0],
                            expectedArgType.IsByRef ? expectedArgType.GetElementType() : expectedArgType);
                    ...
                    {
                        throw new ArgumentException(
                            $"Type mismatch in parameter {i} passed to {method.Name}: expected {expectedArgType}, got {actualArgType}.");
                    }
                }
            }
            ...
            protected TryExpectedExceptionResult TryExpectedException(ExecutionContext context, Action action,
                string contextName, Func<Type, bool> expectedException, string expectedExceptionName,
                bool isTargetException, bool requireException = true)
            {
                ...
                else if (caughtType != null)
                {
                    context.CurrentResult.SetResult(ResultState.Failure,
                        $"In {contextName} code, expected {expectedExceptionName} but got {caughtType.Name}. Exception: {caughtException}");
                    return TryExpectedExceptionResult.ThrewUnexpectedException;
                }
                else
                {
                    context.CurrentResult.SetResult(ResultState.Failure,
                        $"In {contextName} code, expected {expectedExceptionName} but no exception was thrown");
                    return TryExpectedExceptionResult.ThrewUnexpectedException;
                }
            }
            ...
            protected bool CheckExpectedDiagnostics(ExecutionContext context, string contextName)
            {
                ...
                {
                    context.CurrentResult.SetResult(ResultState.Failure,
                        $"In {contextName} code, expecting diagnostic(s) to be logged with IDs {GetDiagnosticIds(_expectedDiagnosticIds)} but instead the following diagnostic(s) were logged: {GetDiagnosticIds(loggedDiagnosticIds)}");
                    return false;
                }
                    ...
                    if (line.StartsWith(matchName))
                    {
                        if (resultNative.GetType() == typeof(Single))
                        {
                            unsafe
                            {
                                ...
                                {
                                    Assert.Fail(
                                        $"Deterministic mismatch '{method.FullName}: {resStr}' but expected '{line}'");
                                }
                            }
                        }
                        else
            ...
            protected abstract unsafe Delegate CompileDelegate(ExecutionContext context, MethodInfo methodInfo,
        ...
        {
            public TestCompilerCommand(TestCompilerAttribute attribute, Test test, TestMethod originalMethod) : base(
                attribute, test, originalMethod)
            {
            }
            ...
            protected override TestResult HandleCompilerException(ITestExecutionContext context, MethodInfo methodInfo)
            {
                ...
                var delegateType =
                    CreateNativeDelegateType(methodInfo.ReturnType, nativeArgTypes, out isInRegistry, out caller);
                ...
                if (functionDelegate == compiledFunction)
                    ...
                    context.CurrentResult.SetResult(ResultState.Failure,
                        $"The function have been compiled successfully, but an error was expected.");
